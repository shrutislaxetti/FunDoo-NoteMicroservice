...
@Repository
public class LabelElasticRepositoryImpl implements LabelElasticRepository {
	...
	public List<Label> findAllByLabelName(String labelName) throws ElasticsearchFailException {
		...
		try {
			...
			throw new ElasticsearchFailException("Fail to get label");
		}
	...
	public Optional<Label> findByLabelNameAndUserId(String labelName, String userId) throws ElasticsearchFailException {
		...
		BoolQueryBuilder boolQuery = QueryBuilders.boolQuery().must(QueryBuilders.termQuery("labelName", labelName))
		...
		try {
			...
			SearchHits hits = searchResponse.getHits();
			SearchHit[] search = hits.getHits();
			for (SearchHit searchHit : search) {
				optionalLabel = Optional.of(objectMapper.convertValue(searchHit.getSourceAsMap(), Label.class));
			}
