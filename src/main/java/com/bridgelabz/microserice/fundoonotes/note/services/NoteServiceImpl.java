...
import com.bridgelabz.microserice.fundoonotes.note.repositories.LabelElasticRepository;
import com.bridgelabz.microserice.fundoonotes.note.repositories.LabelRepository;
import com.bridgelabz.microserice.fundoonotes.note.repositories.NoteElasticRepository;
...
public class NoteServiceImpl implements NoteService {
	...
	@Autowired
	private NoteElasticRepository noteElasticsearchRepository;
	...
	public NoteDTO createNote(CreateNote newNote, String userId)
		...
		List<Label> userLabels = labelElasticRepository.findAllByUserId(userId);
		...
		for (int i = 0; i < labelNameList.size(); i++) {
			if (labelNameList.get(i) != null || labelNameList.get(i).trim().equals("")) {
				...
				if (!userLabelList.contains(labelNameList.get(i))) {
					...
					List<Label> optionalLabelToSave = labelElasticRepository
	...
	public void updateNote(UpdateNote updateNote, String userId, String noteId)
		...
		Optional<Note> optionalNote = noteElasticsearchRepository.findByNoteIdAndUserId(noteId, userId);
	...
	public void addLabel(String userId, String noteId, String labelName)
		...
		Optional<Label> optionalLabel = labelElasticRepository.findByLabelNameAndUserId(labelName, userId);
